...
void fishUndulationPointPatchVectorField::updateCoeffs()
{
    ...
    // changed to the following for OpenFOAM-4.x
	const polyMesh& mesh = this->internalField().mesh()();
	const Time& t = mesh.time();


	Info << "Using amplitude function A = " << a_ << "*x² + " << b_ << "*x + " << c_ << endl;
	Info << "!!! devel version !!!" << endl;
	...
	scalar phi = (constant::mathematical::twoPi * length_) / waveLength_;
...
// *****************************************************************************
	...
	forAll (centerLineOriginal,i2)
	{
		...
		centerLineDeformed[i2][1] = (a_ * pow(centerLineDeformed[i2][0], 2) + b_*centerLineDeformed[i2][0] + c_) * sin(phi * centerLineDeformed[i2][0] - constant::mathematical::twoPi * frequency_ * t.value());
