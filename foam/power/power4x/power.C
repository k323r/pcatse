...
Foam::wordList Foam::functionObjects::power::createFileNames
(
...
) const
{
    ...
    const word powerType(dict.lookup("type"));
    names.append(powerType);
...
void Foam::functionObjects::power::writeFileHeader(const label i)
{
    if (i == 0)
    {
        ...
        if (localSystem_)
        {
            file(i)
                << tab
                << "localPower(pressure,viscous,porous)";
        }
    }
    else if (i == 1)
    {
        ...
        for (label j = 0; j < nBin_; j++)
        {
            ...
            const word f("power" + jn + "[pressure,viscous,porous]");
        ...
        if (localSystem_)
        {
            ...
            {
                const word jn('(' + Foam::name(j) + ')');
                const word f("localPower" + jn + "[pressure,viscous,porous]");
...
void Foam::functionObjects::power::initialise()
{
    if (initialised_)
    {
        return;
...
Foam::functionObjects::power::devRhoReff() const
{
...
Foam::tmp<Foam::volScalarField> Foam::functionObjects::power::mu() const
{
...
Foam::tmp<Foam::volScalarField> Foam::functionObjects::power::rho() const
{
    if (rhoName_ == "rhoInf")
    {
...
Foam::scalar Foam::functionObjects::power::rho(const volScalarField& p) const
{
...
void Foam::functionObjects::power::applyBins
(
...
void Foam::functionObjects::power::writePower()
{
    Log << type() << " " << name() << " write:" << nl
        << "    sum of power:" << nl
        << "        pressure : " << sum(power_[0]) << nl
        << "        viscous  : " << sum(power_[1]) << nl
        << "        porous   : " << sum(power_[2]) << nl
        << endl;
    ...
    if (localSystem_)
    {
        vectorField localPowerN(coordSys_.localVector(power_[0]));
        vectorField localPowerT(coordSys_.localVector(power_[1]));
        vectorField localPowerP(coordSys_.localVector(power_[2]));

        writeTime(file(0));
        file(0) << tab << setw(1) << '('
            << sum(localPowerN) << setw(1) << ' '
            << sum(localPowerT) << setw(1) << ' '
            << sum(localPowerP) << setw(3) << ')'
            << endl;
    }
}


void Foam::functionObjects::power::writeBins()
{
    if (nBin_ == 1)
    {
        return;
    ...
    if (localSystem_)
    {
        ...
        lf[0] = coordSys_.localVector(power_[0]);
        lf[1] = coordSys_.localVector(power_[1]);
        lf[2] = coordSys_.localVector(power_[2]);
...
bool Foam::functionObjects::power::read(const dictionary& dict)
{
    ...
    directForceDensity_ = dict.lookupOrDefault("directPowerDensity", false);
...
void Foam::functionObjects::power::calcPower()
{
    initialise();
